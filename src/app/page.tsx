    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {fixedPositions.map((pos, i) => (
          <motion.div
            key={i}
            className="absolute w-3 h-3 bg-gradient-to-r from-eduvos-innovation to-eduvos-electric rounded-full opacity-25"
      ...
      {/* 🌈 Pulsing Energy Orbs */}
      <motion.div
        className="absolute top-20 left-10 w-80 h-80 bg-gradient-to-r from-eduvos-innovation/30 to-purple-600/20 rounded-full blur-3xl"
        animate={mounted ? {
          scale: [1, 1.4, 1],
          opacity: [0.2, 0.4, 0.2],
        } : {}}
      ...
      />
      <motion.div
        className="absolute bottom-20 right-10 w-96 h-96 bg-gradient-to-r from-blue-600/20 to-eduvos-electric/30 rounded-full blur-3xl"
        animate={mounted ? {
          scale: [1.3, 1, 1.3],
          opacity: [0.3, 0.1, 0.3],
        } : {}}
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4 text-center">
        ...
        >
          ...
          >
            <div className="w-28 h-28 bg-gradient-to-br from-eduvos-electric via-eduvos-innovation to-purple-600 rounded-3xl flex items-center justify-center shadow-2xl shadow-eduvos-innovation/50 relative overflow-hidden">
        ...
        >
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-eduvos-innovation/20 to-eduvos-electric/20 rounded-2xl border border-eduvos-innovation/30 backdrop-blur-lg"
          >
        ...
        <motion.div
          ...
          className="grid md:grid-cols-3 gap-8 mb-16 w-full max-w-5xl"
        >
          {[
            { 
              icon: '💡', 
            ...
            <motion.div
              ...
              className={`relative overflow-hidden rounded-3xl p-8 backdrop-blur-lg border ${item.border} bg-gradient-to-br ${item.gradient} shadow-2xl hover:shadow-3xl transition-all duration-300`}
            >
              {/* Background shine */}
              <motion.div
        ...
        >
          <Link href="/login" className="relative group">
            ...
            >
              ...
              <button className="relative bg-gradient-to-r from-eduvos-electric to-eduvos-innovation text-white px-12 py-5 rounded-2xl font-bold text-xl shadow-2xl group-hover:shadow-3xl transition-all duration-300 flex items-center gap-3">
                <motion.span
          ...
          <Link href="/about">
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="bg-white/10 backdrop-blur-lg text-white px-12 py-5 rounded-2xl font-bold text-xl border-2 border-white/20 hover:border-white/40 hover:bg-white/20 transition-all duration-300 flex items-center gap-3 shadow-2xl hover:shadow-3xl"
            >
              <span>✨</span>
              Discover More
            </motion.button>
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              ...
              >
                <div className={`bg-gradient-to-br ${item.color} rounded-2xl p-1 shadow-lg mb-4 inline-block`}>
                  <div className="bg-eduvos-deep/90 rounded-xl p-4 backdrop-blur-lg">
                    <div className="text-2xl mb-2">{item.icon}</div>
                    <motion.div
                      className="text-3xl font-bold bg-gradient-to-r from-white to-gray-200 bg-clip-text text-transparent"
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
